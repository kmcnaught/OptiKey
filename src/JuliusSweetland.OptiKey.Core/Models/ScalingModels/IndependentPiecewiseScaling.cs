...
{
    class IndependentPiecewiseScaling : ISensitivityFunction
    {
        List<Tuple<Tuple<float, float>, Tuple<float, float>>> pairwiseCoordsX;
        List<Tuple<Tuple<float, float>, Tuple<float, float>>> pairwiseCoordsY;

        List<Tuple<float, float>> individualCoordsX;
        List<Tuple<float, float>> individualCoordsY;

        public IndependentPiecewiseScaling()
        {
        }

        public void SetCoordsX(float[] list_of_all_coords)
        {
            SetCoords(list_of_all_coords, ref individualCoordsX, ref pairwiseCoordsX);
        }

        public void SetCoordsY(float[] list_of_all_coords)
        {
            SetCoords(list_of_all_coords, ref individualCoordsY, ref pairwiseCoordsY);
        }

        public void SetCoords(float[] list_of_all_coords, 
                ref List<Tuple<float, float>> individualCoords,
                ref List<Tuple<Tuple<float, float>, Tuple<float, float>>> pairwiseCoords)
        {
            ...
            if (n > 0)
            {
                ...
                if (individualCoords.Last().Item1 <= 1.0f)
                {
        ...
        public Vector CalculateScaling(Point current, Point centre)
        {
            ...
            distance.X /= (Graphics.PrimaryScreenWidthInPixels/2.0);
            distance.Y /= (Graphics.PrimaryScreenHeightInPixels / 2.0);
