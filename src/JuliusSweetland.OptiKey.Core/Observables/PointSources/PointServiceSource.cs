...
namespace JuliusSweetland.OptiKey.Observables.PointSources
{
    public class PointServiceSource : IPointSource
    {
        ...
        private readonly SmoothingEffect smoothingEffect;
            ...
            IPointService pointGeneratingService)
        {
            ...
            this.smoothingEffect = new SmoothingEffect();
        }
                ...
                if (sequence == null)
                {
                    sequence = Observable.FromEventPattern<Timestamped<Point>>(
                        ...
                        .Select(ep => Settings.Default.GazeSmoothingLevel > 0
                            ? new Timestamped<Point>(smoothingEffect.Update(ep.EventArgs.Value, ep.EventArgs.Value.ToKeyValue(PointToKeyValueMap)), ep.EventArgs.Timestamp)
                            : ep.EventArgs
                        )
