...
using static JuliusSweetland.OptiKey.Models.XInputListener;

namespace JuliusSweetland.OptiKey.Observables.TriggerSources
{
    public class GamepadButtonDownUpSource : ITriggerSource
    {
        #region Fields
        ...
        public GamepadButtonDownUpSource(
            ...
            IPointSource pointSource)
        {
            ...
            xinputListener = new XInputListener(UserIndex.One);
        }
        ...
        public IObservable<TriggerSignal> Sequence
        {
            get
            {
                if (sequence == null)
                {
                    //FIXME: can I avoid the eventargs hacks?
                    var keyDowns = Observable.FromEventPattern<EventHandler, EventArgs>(
                            handler => new EventHandler(handler),
                            h => xinputListener.ButtonDown += h,
                            h => xinputListener.ButtonDown -= h)
                        .Do(ep => {
                            GamepadButtonDownEventArgs buttonState = (GamepadButtonDownEventArgs)ep.EventArgs;
                            Log.InfoFormat("gamepad button {0} UP", buttonState.button);
                        })
                        .Where(ep => {
                            GamepadButtonDownEventArgs buttonState = (GamepadButtonDownEventArgs)ep.EventArgs;
                            return (buttonState.button == triggerButton);
                         })
                        .Select(_ => true)
                        .Do(_ => Log.DebugFormat("Trigger key down detected ({0})", triggerButton));

                    var keyUps = Observable.FromEventPattern<EventHandler, EventArgs>(
                            handler => new EventHandler(handler),
                            h => xinputListener.ButtonUp += h,
                            h => xinputListener.ButtonUp -= h)
                        .Do(ep => {
                            GamepadButtonUpEventArgs buttonState = (GamepadButtonUpEventArgs)ep.EventArgs;
                            Log.InfoFormat("gamepad button {0} UP", buttonState.button);
                        })
                        .Where(ep => {
                            GamepadButtonUpEventArgs buttonState = (GamepadButtonUpEventArgs)ep.EventArgs;
                        ...
                        .Select(_ => false)
                        .Do(_ => Log.DebugFormat("Trigger key up detected ({0})", triggerButton));
